...
Единый портфель **проектов и продуктов** со сквозным приоритетом (RICE), очередями команд, Gantt и планированием ёмкости.
...
| Вкладка | Смысл |
|---|---|
| **Портфель** | Проекты + продукты, RICE, ручной приоритет |
| **Gantt/Сроки** | ETA по weekly capacity, опционально загрузка команд |
| **Очередь команд** | Сквозной приоритет, «может взять с…», тоггл авто-сдвига по ёмкости |
| **Настройки** | Команды (ёмкость), диапазоны майок S/M/L, «сброс демо» (шутка) |
